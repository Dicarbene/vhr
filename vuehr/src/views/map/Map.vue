<template>
    ...
    70%">
                ...
                <el-table-column prop="createDate" width="150" label="创建时
       间"></el-table-column>
            ...
            <el-button @click="deleteMany" type="danger" size="small" style="margin-top: 8px"
                :disabled="multipleSelection.length == 0"> 批量
                删除</el-button>
        </div>
        <el-dialog title="地图展示" :visible.sync="dialogVisible_show" width="80%">
            ...
            <span slot="footer" class="dialog-footer">
                <el-button size="small" @click="dialogVisible_show = false">
                    退出</el-button>
            </span>
        </el-dialog>
        <el-dialog title="增加地图" :visible.sync="dialogVisible_add" width="40%">
            ...
            <span slot="footer" class="dialog-footer">
                <el-button size="small" @click="dialogVisible_add = false">
                    取 消</el-button>
...
export default {
    ...
    methods: {
        ...
        async uploadImg() {
            ...
            if (this.inputFile) {
                ...
                if (inputFile.type !== 'image/jpeg' &&
                    inputFile.type !== 'image/png' && inputFile.type !==
                    'image/gif') {
                    alert('不是有效的图⽚⽂件！');
                    return;
                }
                ...
                reader.onload = function (e) {
                    console.log(e);
                    that.AddMap.url = this.result;// 注意:这⾥的 this.result 中,这个 this 不是 vue 实例,⽽是 reader 对象,所以之前⽤ that 接收 vue 示例
                    that.imgSrc
        ...
        addMap() {
            this.postRequest("/map/manage/", this.AddMap).then(resp => {
                if (resp) {
                    this.initMaps();
